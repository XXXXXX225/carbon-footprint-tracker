...
const completedItems = computed(() => historyItems.value)
const pendingItems = computed(() => planItems.value)
const completionRate = computed(() => {
  const total = planItems.value.length + historyItems.value.length
...
const saveHistoryItems = () => {
  localStorage.setItem(HISTORY_TASKS_KEY, JSON.stringify(historyItems.value))
}
const loadHistoryItems = () => {
  try {
    const raw = localStorage.getItem(HISTORY_TASKS_KEY)
...
const buildPlanItems = () => {
  ...
  planItems.value = baseItems.slice(0, 5)
}

const loadData = async () => {
  loading.value = true
  historyItems.value = loadHistoryItems()
...
const confirmReopenTask = (task: PlanItem) => {
  ElMessageBox.confirm(
    ...
    const index = historyItems.value.findIndex((item: PlanItem) => item.id === task.id)
    if (index > -1) {
